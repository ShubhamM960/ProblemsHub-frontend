...
class Home extends Component {
  ...
  handleSearch = (event) => {
    ...
    const url =
      ...
      request(
        `https://localhost:8080/suggest/${this.state.userHandle.trim()}/${
          this.state.counts.easy
        }/${this.state.counts.medium}/${this.state.counts.hard}` +
          (this.state.ratingLow
            ? `/${this.state.ratingLow}/${this.state.ratingHigh}`
            : ``),
        (error, response, body) => {
  ...
  startTimer = (problem, diff) => {
    ...
    this.setState({
      showProblemPage: false,
      showStopwatch: true,
      ongoingProblem: dProb
    });
  };

  stopTimer = () =>
    this.setState({ showStopwatch: false, showProblemPage: true });

  solved = (diff, cid, index) =>
    this.state.problemSet[diff].forEach((prob, idx) => {
      ...
      <div className="home">
        {!this.state.showProblemPage && !this.state.showStopwatch ? (
          <div className="hello">
            ...
            <div data-text="Welcome..." className="welcome">
              Welcome...
            </div>
          </div>
        ) : null}
          ...
          <div className="full-page show-page" id="#main">
            <div>
              <img
                ...
                src={`https:${this.state.userPic}`}
                alt={this.state.userHandle}
              />
              <span className="user">
                <span>
                  <a
                    href={`http://codeforces.com/profile/${this.state.userHandle}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      color: rankColor(this.state.userRating),
                      textDecoration: "none"
                    }}
                  >
                ...
                <span className="ratingBadge">
                  <NotificationBadge
                    count={this.state.userRating}
                    style={{
                      backgroundColor: rankColor(this.state.userRating),
                      color: "#1d1e22",
                      top: "0",
                      right: "0"
                    }}
                ...
                >
                  <IconContext.Provider
                    value={{ color: "orange", size: "2em" }}
                  >
                    <AiOutlineUserDelete />
                  </IconContext.Provider>
                </span>
              </span>
              <div style={{ fontSize: "18px", color: "#a58787" }}>
                {this.state.userFName} {this.state.userLName}
              </div>
              <span style={{ fontSize: "18px", color: "rgb(189 104 104)" }}>
            ...
            {this.state.problemSet.easy === undefined ? null : (
              <>
                <h2 style={{ color: "#8f8e2f" }}>
                  Unsolved problems from your last contest
                </h2>
                <div className="Suggestions1">
                  {this.state.problemSet.solveNext
                    .sort((a, b) => a["solvedBy"] < b["solvedBy"])
                    .map((problem) => (
                      <Card
                        handle={this.state.userHandle}
                        problem={problem}
                        difficulty="solveNext"
                        startTimer={this.startTimer}
                      />
                    ))}
                </div>
                <h2 style={{ color: "#8f8e2f" }}>
                  Suggested problems from contests you have taken part in
                </h2>
                <div>
                  <span>
                ...
                <div>
                  <h2 style={{ color: "#817b18e0" }}>
                    Suggested Practice problems from past contests
                    <a href="#settings" className="seticon">
                      <IconContext.Provider
                        value={{ size: "1em", color: "#992636" }}
                      >
                        <FiSettings />
                      </IconContext.Provider>
                    </a>
                ...
                {this.state.counts.easy +
                  this.state.counts.medium +
                  this.state.counts.hard >
                  0 && this.state.ratingLow <= this.state.ratingHigh ? (
        ...
        {!this.state.showStopwatch ? null : (
          <Timer
            problem={this.state.ongoingProblem}
            solved={this.solved}
        ...
        <div id="settings" className="popup">
          ...
          <input
            ...
            step="50"
            required
            value={this.state.ratingLow}
            onChange={(e) => this.setState({ ratingLow: e.target.value })}
          />
          -
          <input
            className="toggler"
            ...
            step="50"
            required
            value={this.state.ratingHigh}
            onChange={(e) => this.setState({ ratingHigh: e.target.value })}
          />
          <hr />
          Problem Counts :&nbsp;
          <input
            ...
            name="easy"
            required
            value={this.state.counts.easy}
            onChange={(e) =>
              this.setState({
                counts: { ...this.state.counts, easy: e.target.value }
              })
            }
          />
          <input
            ...
            name="medium"
            required
            value={this.state.counts.medium}
            onChange={(e) =>
              this.setState({
                counts: { ...this.state.counts, medium: e.target.value }
              })
            }
          />
          <input
            ...
            name="hard"
            required
            value={this.state.counts.hard}
            onChange={(e) =>
              this.setState({
                counts: { ...this.state.counts, hard: e.target.value }
              })
            }
          />
          <hr />
